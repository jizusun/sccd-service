			...
			if(oFile.isExist()){
				...
				*/
				...
				oFile.readLineByLine(function(sLine){
					if(this.getProjectType() === Project.Type.BackEnd){
						var oRegExp = new RegExp(/^\s*<testsuite name=".*" tests="(\d+)" failures="(\d+)" errors="(\d+)" skipped="(\d+)"/i);
						var aMatchKpi = oRegExp.exec(sLine);
						if(aMatchKpi){	
							if(parseInt(aMatchKpi[1]) > 0){
								this.oKpi[sTestType].assertion += (parseInt(aMatchKpi[1]) - parseInt(aMatchKpi[4]));
								this.oKpi[sTestType].passed += (parseInt(aMatchKpi[1]) - parseInt(aMatchKpi[2]) - parseInt(aMatchKpi[3]) - parseInt(aMatchKpi[4]));
								this.oKpi[sTestType].failed += (parseInt(aMatchKpi[2]) + parseInt(aMatchKpi[3]));
								this.oKpi[sTestType].skipped += parseInt(aMatchKpi[4]);
							}
						}
					}

					else{
						oRegExp = new RegExp(/^\s*<testsuites.*tests="(\d+)"/i);
						aMatchKpi = oRegExp.exec(sLine);
						if(aMatchKpi && parseInt(aMatchKpi[1]) > 0)
							this.oKpi[sTestType].assertion = parseInt(aMatchKpi[1]);

						oRegExp = new RegExp(/^\s*<testcase/i);
						if(oRegExp.exec(sLine))
							this.oKpi[sTestType].passed++;

						oRegExp = new RegExp(/^\s*<failure/i);
						if(oRegExp.exec(sLine)){
							this.oKpi[sTestType].passed--;
